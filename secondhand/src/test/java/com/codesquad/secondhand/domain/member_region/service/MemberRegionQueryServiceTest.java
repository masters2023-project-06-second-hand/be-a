...
class MemberRegionQueryServiceTest {
	...
	void findByMemberAndRegionFailed() {
		...
		// when & then
		assertThatThrownBy(() -> memberRegionQueryService.findByMemberAndRegion(notSavedMember,savedRegion)).isInstanceOf(CustomRuntimeException.class);
		assertThatThrownBy(() -> memberRegionQueryService.findByMemberAndRegion(savedMember,notSavedRegion)).isInstanceOf(CustomRuntimeException.class);
	}
